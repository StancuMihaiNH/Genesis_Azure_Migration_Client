...
import TopicPinningButton from "@/components/TopicPinningButton/TopicPinningButton";
...
import useMessageStore, { handleGenerateAITitle, handleSendAndStreamCallback } from "@/store/useMessageStore";
import { newID } from "@/utils/id";
import classNames from "classnames";
import { getUnixTime } from "date-fns";
import dynamic from "next/dynamic";
import React, { useContext, useEffect, useMemo, useRef, useState } from "react";
import { RiAddFill, RiArrowDownSLine, RiArrowUpSLine, RiCloseLine } from "react-icons/ri";
import { IConversationProps, ITagContainerProps, ITagSectionProps } from "./Conversation.types";

const ComposeInput = dynamic(() => import("@/components/ComposeInput/ComposeInput"), {
  ssr: false,
});

const TagSection = (props: ITagSectionProps): JSX.Element => {
      ...
      {open && (
        <div className={classNames("flex flex-col flex-wrap gap-2")}>
          {props.tags.map((tag: Tag, index: number): JSX.Element => (
            <button
              onClick={(): void => props.onClick(tag)}
...
const TagContainer = (props: ITagContainerProps): JSX.Element => {
  ...
  const parts: Array<string> = props.text
    .split("_")
    .slice(0, 2)
    .map((part): string => { return part.charAt(0).toUpperCase() + part.slice(1).toLowerCase(); });
...
const Conversation = (props: IConversationProps) => {
  ...
  const { data: messagesData, client, refetch: onRefetch, loading: messagesLoading } =
    useMessagesQuery({
      variables: {
        topicId: props.id
      }
  ...
  const endRef: React.RefObject<HTMLDivElement> = React.useRef<HTMLDivElement>(null);
  ...
  const availableSectionTags = useMemo(() => {
    const mapCategoryById: Map<string, Category> = new Map<string, Category>();
    for (const category of categoriesData?.categories || []) {
      mapCategoryById.set(category?.id ?? "", category as Category);
    }

    let categories: Map<string, Tag[]> = new Map<string, Tag[]>();
    for (const tag of tags) {
      let categoryId: string = tag?.categoryId || "General";
    ...
    return Array.from(categories.entries()).map(([category, tags]) => (
      <TagSection
        ...
        onClick={(tag): void => {
          ...
          updateTopic({
            variables: {
              updateTopicId: props.id,
              input: {
                // @ts-ignore
                tagIds: [...(topic.tags || []), tag].filter((t): boolean => t !== undefined).map((t) => t?.id)
              }
            },
            optimisticResponse: {
              updateTopic: {
  ...
  const updateMessageToCache = (message: Message): void => {
  ...
  const handleResend = async (message: Message): Promise<void> => {
    if (!topic) {
      return;
    }
      ...
      handleSendAndStreamCallback(
        ...
        (res, error) => {
          ...
          if (res === "__done__") {
            ...
            let content: string | null | undefined = messageFragment.content;
            const regex: RegExp = /^\[.*?}(?=])/;
  ...
  const handleSend = async (msg: string, files?: File[]): Promise<void> => {
    if (!topic) {
      return;
    }

    const newMessageId: string = newID();
    try {
      setIsSending(true);
      abortControllerRef.current = new AbortController();
  ...
  return (
    <div
      className={classNames(
        "flex flex-1 flex-col h-screen max-w-full overflow-x-hidden",
        props.className,
      )}
    >
      <div className={"shadow w-full p-4"}>
        <div className={"flex flex-col gap-2 justify-start items-start"}>
          <div className="flex-col md:flex-row flex justify-between w-full gap-4">
            <div
              className={
                "text-[#132e53] text-sm md:text-lg font-semibold flex items-center gap-2"
              }
            >
              <ToggleSidebarButton />
              {!showEditTitle ? (
                ...
                <input
                  ...
                  onBlur={() => {
                    handleUpdateTopic(props.id, { name: title });
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <div className={"flex gap-2 items-center"}>
                ...
                <TopicPinningButton topicId={props.id} />
              </div>
            </div>
          </div>
      ...
      >
        {messages.map((message) => (
          <MessageCard
            ...
            handleResend={() => handleResend(message as Message)}
            onRefetch={onRefetch}
