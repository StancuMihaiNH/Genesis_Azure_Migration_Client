import { Prompt, PromptsDocument, useCreatePromptMutation, useDeletePromptMutation } from "@/graphql/__generated__/schema";
import classNames from "classnames";
import React, { useState } from "react";
import { useForm } from "react-hook-form";
import { BiTrash } from "react-icons/bi";
import { FormValues, IPromptsProps } from "./Prompts.types";
...
const Prompts = (props: IPromptsProps) => {
  ...
  return (
    <div className={
      ...
      {props.prompts.map((prompt: Prompt, index: number): JSX.Element => (
        <div onClick={(): void => props.onPromptSelect?.(prompt.description ?? "")}
          key={prompt.title}
            ...
            <button className={"p2"}
              onClick={(e): void => {
                ...
                client.writeQuery({
                  query: PromptsDocument,
                  data: {
                    prompts: readQuery.prompts.filter((p: Prompt): boolean => p.id !== prompt.id)
                  }
                });
              }} >
      ...
      <div ref={ref}>
        {!showAddPrompt && (
          <button onClick={(): void => {
            ...
            className={"w-full flex flex-col gap-1 p-2 hover:bg-gray-100 transition cursor-pointer border-b text-[#132e53] font-semibold"} >
            Add new Prompt
          </button>
        )}
        {showAddPrompt && (
